...
contienente_origen(abrojo, america).
...
contienente_origen(oregano, europa).
...
contienente_origen(palo_de_flor, america).
...
accion_efecto_planta(palo_de_flor, analgésico).
...
contienente_origen(pasiflora, america).
...
accion_efecto_planta(pasiflora, ansiolítico).
